...
# Identity ID
$IdPattern = '^[a-zA-Z0-9]{7}$'
do {
    $IdentityID = Read-Host -Prompt "Enter Identity ID (e.g. abc1234)"
    if ($IdentityID -notmatch $IdPattern) { Write-Host "Invalid format! Must be exactly 7 alphanumeric chars." -ForegroundColor Red }
} while ($IdentityID -notmatch $IdPattern)
$IdentityURL = "https://${IdentityID}.id.cyberark.cloud"

# Tenant URL
$UrlPattern = '^https://[\w-]+\.cyberark\.cloud$'
do {
...
[PSCustomObject]@{
    Login             = $Username
    User_Password     = '********' # Masked in export for security, or change to $Password if you strictly need it in plaintext
    PAM_SaaS_Url      = $PAMUrl
...
# ==============================================================================
Write-Host "`n--- Provisioning Training Resources ---" -ForegroundColor Cyan

# Create Training User
$body = @{
    Name = $TrainingUser
    Mail = "mlaget+Lab@paloaltonetworks.com"
...
if (-not [string]::IsNullOrWhiteSpace($PrivCloudAdmins)) {
    ...
    Write-Host "[-] Failed: Could not find 'Privilege Cloud Administrators' via Redrock query." -ForegroundColor Yellow
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAWHxQTq4Ec7Qk9
# Wb2pJBv9/DqTR9jl0MKnfVN7dqDW1KCCGNIwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# AQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIPtMttRf+bXYVlhz6VH15Eot7y60/wg3
# RkHdqyTcR34SMA0GCSqGSIb3DQEBAQUABIIBABJVVJpx3dcxR90IlnU2raCzNZRO
# YMw3pIm7/6W+F5pYL6djkSisosEhFwIfEsKRLlH3gn4AwsSCFSoLBGVvAF6CVuBs
# iq1PzDppiVW2qFJhDIpDI8MyYSp7d32EhcvYhcada3uAFjxHmm6X5Koov8EDTHtY
# 4n7HthdjL3s4UgYsAeLwGQX58eU9FEhFWnAxaRAi5gzUnnnPAhCuYkdpmsiX8i1I
# XHE257uMkPfdB83Bsm922jRBU7bwQ5dChYXZCWE2g7xpaSvTpMMS8YmZyLwjWpDH
# cKxZmsSoh3GvJSl6z7Qq/iKe+q7Y7S3b40Hrj7OXn3oyPKfdWY71hI/lVeyhggMm
...
# DQEHATAcBgkqhkiG9w0BCQUxDxcNMjYwMzE4MDAzMDM3WjAvBgkqhkiG9w0BCQQx
# IgQggRQcjWWa/JLsHgKQ3ZoU96YmboWaFRZgoyFfbbwS1KIwDQYJKoZIhvcNAQEB
# BQAEggIAJQ45JXLAlQ/qKnwaZ1+89ypEnVDR6H+bymb1f2VRd5PvRwLniyQhmJXY
# fsq9cjjVrTkQUtAF/32G5bFn+R1reRrgPeFVgQvIC7ZUIIL5C/kQls99D4Fx648g
# tyrQxn8eNxKeGvNbRJfdnHhztzPMw3JsxC57mcBZaae7GOmvSa4plwKC/vCHGR3C
# /qkt1hYzQD7wqYWi1+RXJMMFNvE0Y4V8s0gfqc4RoFqkwYr3BsBMrNut0RzTeEjA
# 3n+sX/v9lA/eZsG2nA0yO+XnehmNi3dVishVOuWTRnqlxBrAgC+gwhaEkqwm5IZA
# JKQYvKl/RJiZBIBVZrEGyvto/cWbtJ56lCWQDaph7Fxl89WoBk7TehOraAj9EHVt
# VIeUdBvz91SFVyilR6NamuAUjFqrNGSukIyrEon3NddzjEUGt8GNnnF7XLLZT9l8
# X9TIFu/siOyiyL477dgIi+4J1eo4RxozrQ2fQV5W20v5bdkbYfAGh4/ehG77x1ko
# eaBVdOu0k3BM4R1GfMYzLoGosusB8E5wXlY2rUEsK6cUVmH+XNVmIxvl04yo/kzy
# bFRLnbOB5vxeTJeETBAW8LqPzfjVRSBscr43Qxy61YtiTNK6i/GHcySMTH8KGMov
# MPGBykPHMAk3LujDVSspZUmSXCUxzGKGDki9K+a8sCBRefRdVKY=
