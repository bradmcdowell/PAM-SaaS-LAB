#This script is aimed at creating the the Account used to scan the targets of the discovery.
...
# 1. Import the module containing reusable functions
Import-Module "$PWD\AccountProvisioningHelper.psm1"

# 2. Start logging (transcript)
Start-Transcript
...
$AccountsToCreate = @("Acme_DOM_Scanner")
...
foreach ($Account in $AccountsToCreate) {
    ...
    if ($existingAccount) {
        ...
        # If the Account exist, ask the user if he wants to delete the current version and create a new Account.
        ...
        if ($response -match '^[Yy]$') {
            try {
                ...
                #An error occurred and the account couldn't be created.
                Write-Error "Failed to delete existing Account '$Account': $_"
                continue  # Skip to next Account
            }
        } else {
            #If user doesn't want to recreate the account, the loop will skip to the next account in the list
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC4Apxv4F8Mb+Ij
# PYyJ51XNKcMLHZ/5slYc4+LgB93YoqCCGNIwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# AQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIFJhpPH7WVbsqzD/thiH9c5qBAu1qslm
# kY0KwuhOK5YDMA0GCSqGSIb3DQEBAQUABIIBAANFDKEO5zcGjGyojckxBzJrQniz
# a9QTPvpXjvgDhi98fshKrvsBxmRIppyg0HiihVqVRvUibHR1RdNeyxaSCO/spkIe
# spOa7C1E2CRQl/vktPZaJsn+unXWrb+UnxPKOJTxPNjqAO5lKEUoc2FL3TtbPJvE
# 71Q+zos25HeX1cziV0z+mTbsvnLI/Meclu6g66K2BlIi2IFgjNH+7yGmOfBhStP0
# I9l3DHEm6stgAmAVq4VH0NH528GlDImWVzgGGpSa25RVLpnYhwGdqwhUPqqH2xZn
# 2swWCGXrEhiNcgnmsC1Uk/uGeBhZbMzfB/aCL9MUX3dmo9XBJfhPeg62JKihggMm
...
# DQEHATAcBgkqhkiG9w0BCQUxDxcNMjUxMjA4MjIwNzUzWjAvBgkqhkiG9w0BCQQx
# IgQgAH/JRCjG4E64QQ7QTWqZE3qNJ6key4a8n47INmon1IwwDQYJKoZIhvcNAQEB
# BQAEggIAleYq64oIQ95zx+SW2YmwrK8eMVgqnEEgNRW4JmF3xRnPEwyIxa5e+juU
# mX7N+rmv+p3GaGZBkK1FoBA4V9w+6y6YUPk/bjWHHgvJOMIVrmmLgqjQfqyXG/Iq
# 8kp7mqRsHbLBlQOuFW7Ux9aUHCV/Xh+Fxdawfo/yjNOOMp9peJ1072xjXp+RZpHb
# JgRS5cJjG1iOMC2VNPNT71cGMBFUHbdGMYxIF6Hn01FZIWD+ZAWbAFvsKHzANwau
# qoIIMTelpJYi5zvEOnHsvdpWgLnvUSyertRgWV4AFKXNJPnEQ3+RfZNXFkFxDQn0
# zIjr+88QwmqrC8uGxrAyokSKoeH0XvobXs4RScOQ6XGzYo8Twt/ShFszHZTTOH3E
# WuRMT27OlGnxBexyYIdAhDx8bUFnia6/gBdn9UzYyBLweJEui+EnN1XTuCelcRKl
# ovXMDJeQRwNfrW4GJxBoM+sZjMkcm0o24FjNejp77mEy7ZEzdEA9qlMjWlWrmGgN
# 2BWcvnfdPtDcufZLo4qs4qcD9A0jIdQ2AfJWQChf7BLsWxKVI3gEmjXyMo4dObQh
# QTCJ+NHke2s85i+rTDUbOwxIZR3bjmLcuNuR2iOikDHiLAG5rf+0k0ki+ArwtUaJ
# n/s2gQtuFvZIvQKP1BLFyLtPENgL7A19+bOvwi5iJ+QpWTbafXc=
